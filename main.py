...
def prepare_query(year_value):
    ...
    if year_value in crawl_index.keys():
        return crawl_index[year_value]
    else:
        return []
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        if sys.argv[1] == "init":
            ...
            """
            url_df = pd.read_csv("./input/input.csv", header=None)
            ...
            if len(url_list) > 0:
                ...
                crawl_index.crawl_cc_google(url_list, index_list, limit=250)
                '''
                crawl_index.crawl_cc_google(['1800flowers.com'], index_list, limit=100)
